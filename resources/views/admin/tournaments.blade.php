          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Tounaments</h3>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                   ...
                   <tr>
                        <td id="{{ $tournament->tournament_id }}_name">{{ $tournament->name }}</td>
                        <td><button class="btn btn-primary" onclick="openModel({{ $tournament->tournament_id }}, true)">Edit</button></form></td>
                        <td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addEventModalLabel">Add New Tounament</h5>
...
<script>
    ...
    function openModel(eventID=null, update=false){
      if(update){
        $('#f_method').val('PATCH')
        document.getElementById('addEventModalLabel').innerHTML = 'Edit Tournament Details'
        $('#edit_up_form').attr('action', `${window.location.href}/${eventID}`)
        $('#m_name').val($(`#${eventID}_name`).html())
        $('#m_event_date').val($(`#${eventID}_event_date`).html())
        $('#m_status').val($(`#${eventID}_status`).html())
        $('#addEventModal').modal()
      }else{
        $('#f_method').val('POST')
